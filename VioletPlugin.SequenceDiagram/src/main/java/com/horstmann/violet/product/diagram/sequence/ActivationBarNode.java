...
public class ActivationBarNode extends RectangularNode
{
    ...
    private boolean isCallEdgeAcceptable(CallEdge edge)
    {
        ...
        if (startingNodeClass.isAssignableFrom(ActivationBarNode.class)
                && endingNodeClass.isAssignableFrom(ActivationBarNode.class))
        {
            return edgeDoesNotIntroduceCycle(edge);
        }
    ...
    private boolean edgeDoesNotIntroduceCycle(CallEdge edge) {
    	// Reset marks
    	for (INode n: getGraph().getAllNodes()) {
    		if (n instanceof ActivationBarNode) {
    			((ActivationBarNode) n).visited = false;
    		}
    	}

    	visitEdge(edge);

    	return ((ActivationBarNode) edge.getStart()).visited == false;
	}

    private static void visitEdge(CallEdge edge) {
    	ActivationBarNode end = (ActivationBarNode) edge.getEnd();
    	if (end.visited) return;

    	visitNode(end);
    }

    private static void visitNode(ActivationBarNode node) {
    	if (node.visited) return;

    	node.visited = true;

    	for (INode c: node.getChildren()) {
    		if (c instanceof ActivationBarNode) {
    			visitNode((ActivationBarNode) c);
    		}
    	}

    	for (IEdge e: node.getConnectedEdges()) {
    		if (e.getStart() == node && e instanceof CallEdge) {
    			visitEdge((CallEdge) e);
    		}
    	}
    }
